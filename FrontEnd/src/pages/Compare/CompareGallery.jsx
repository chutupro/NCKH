...
const CATEGORIES = ['Tất cả', 'Văn hóa', 'Kiến trúc', 'Du lịch', 'Thiên nhiên'];
...
const CompareGallery = () => {
  ...
  const filtered = useMemo(() => {
    ...
    if (query && query.trim()) {
      ...
      list = list.filter((i) => i.title.toLowerCase().includes(q) || i.post.toLowerCase().includes(q));
    }
    return list;
  }, [query, category]);
  ...
  return (
    <div className="cc-root">
      <header className="compare-header">
        ...
        <p className="subtitle">So sánh về đẹp qua thời gian của các di sản</p>
      </header>

      <section className="cc-controls">
        <div className="cc-search-wrap">
          <input
            placeholder="Tìm kiếm theo tiêu đề hoặc bài viết..."
            value={query}
        ...
        <div className="cc-filters">
          {CATEGORIES.map((c) => (
            <button
              key={c}
              className={`cc-filter-btn ${category === c ? 'cc-active' : ''}`}
