...
const UserManagement = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      toast.error('Không th? t?i danh sách');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleChangeRole = async (userId, newRoleId) => {
    try {
      ...
      toast.success('Ð? c?p nh?t vai tr?');
      fetchUsers();
      fetchStats();
    } catch (error) {
      toast.error('L?i');
  ...
  const handleToggleStatus = async (userId, currentStatus) => {
    ...
    if (!window.confirm(newStatus === 'locked' ? 'Khóa?' : 'M??')) return;
    try {
      await adminUsersService.updateUser(userId, { status: newStatus });
      toast.success(newStatus === 'locked' ? 'Ð? khóa' : 'Ð? m?');
      fetchUsers();
      fetchStats();
    } catch (error) {
      toast.error('L?i');
  ...
  if (loading && users.length === 0) return <div className="admin-content">Ðang t?i...</div>;

  return (
    <div className="admin-content">
      <div className="admin-header">
        <h1>Qu?n L? Ngý?i Dùng</h1>
      ...
      {pagination.totalPages > 1 && (
        <div className="admin-pagination">
          <button className="admin-pagination-btn" onClick={() => setPagination(prev => ({ ...prev, page: prev.page - 1 }))} disabled={pagination.page === 1}> Trý?c</button>
